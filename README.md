...
M4：新增 `app/core/downloader.py`（依 LINE/Telegram 平台以 `asyncio.gather` 平行下載緩衝區中的照片，個別失敗僅略過不中斷）與 `app/core/vision.py`（以 `google-genai` SDK 呼叫 `gemini-flash-latest` 多模態辨識照片與文字描述，輸出結構化營養素品項）。`ok` 指令已串接下載＋辨識流程；已以真實 GEMINI_API_KEY 與實際照片手動驗證辨識結果格式正確。寫入使用者專屬 Sheet 的 `daily_log` 與清空 buffer 留待 M5，詳見 [TODO.md](TODO.md)。
